...
void MakeJmp(Arch arch, uint8_t* buf, uint64_t cur_addr, uint64_t jmp_addr) {
	...
	case Arch::kX64: {
		...
		a.push(static_cast<uint32_t>(jmp_addr));
		a.mov(dword_ptr(rsp, 4), static_cast<uint32_t>(jmp_addr >> 32));
		a.ret();
...
std::optional<InlineHook> InlineHook::InstallEx(
	...
	if (hook.tls_id_ == TLS_OUT_OF_INDEXES) { return std::nullopt; }

	if (forward_page_size < 0x1000) { forward_page_size = 0x1000; }

	if (instr_size > 255) { return std::nullopt; }


	auto forward_page_res = hook.process_->AllocMemory(NULL, forward_page_size, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	if (!forward_page_res) { return std::nullopt; }
	...
	if (!hook.process_->ReadMemory(hook_addr, temp.data(), 64)) { return std::nullopt; }

	if (instr_size == 0) {
		if (arch == Arch::kX86) { while (instr_size < 5) { instr_size += insn_len_x86_32(&temp[instr_size]); } }
	...
	if (arch == Arch::kX86) {
		if (instr_size < 5) { return std::nullopt; }
		auto a = Assembler(Arch::kX86);
		...
		if (instr_size < 14) { return std::nullopt; }
		auto a = Assembler(Arch::kX64);
		auto next_lab = a.NewLabel();
	...
	if (hook.process_->IsCurrent() &&
		...
		if (arch == Arch::kX86) {
			...
			InterlockedExchange64(reinterpret_cast<volatile int64_t*>(hook_addr),
			                      *reinterpret_cast<int64_t*>(jmp));
		}
		else {
#ifdef _WIN64
			uint8_t buf[16];
			...
			success = InterlockedCompareExchange128(
				reinterpret_cast<volatile int64_t*>(hook_addr),
				*reinterpret_cast<int64_t*>(&jmp[8]),
				*reinterpret_cast<int64_t*>(&jmp[0]),
		...
		uint8_t jmp[16];
		MakeJmp(arch, jmp, hook_addr, hook.forward_page_);
		if (!hook.process_->WriteMemory(hook_addr, jmp, instr_size, true)) { return std::nullopt; }
	}
	return hook;
}
...
std::optional<InlineHook> InlineHook::Install(
	...
	return InstallEx(proc,
	                 hook_addr,
	                 reinterpret_cast<uint64_t>(InstallX32Callback),
	                 Arch::kX86,
	                 instr_size,
	                 save_volatile_register,
	                 forward_page_size);
}
...
std::optional<InlineHook> InlineHook::Install(
	...
	return InstallEx(proc,
	                 hook_addr,
	                 reinterpret_cast<uint64_t>(InstallX64Callback),
	                 Arch::kX64,
	                 instr_size,
	                 save_volatile_register,
	                 forward_page_size);
...
void InlineHook::Uninstall() {
	...
	if (tls_id_ != TLS_OUT_OF_INDEXES) { TlsFree(tls_id_); }
}
}
